...
** 기존 페이지가 없는 경우 물음표가 생기거나, 첫글자가 굵게 표시가 됩니다. (후자는 환경설정에서 "내용이 없는 페이지의 링크를 wikiX 스타일로 표시"에 체크한 경우) 물음표 또는 첫 글자를 클릭하면 새페이지 창이 열립니다.
...
* 페이지 이름과 첫번째 숫자는 includedaya 매크로와 동일하다
* 세번째 인자인 숫자의 갯수 만큼의 "내용이 있는" 페이지를 include 한다.
* 세번째 인자의 부호가 "-" 이면 이전 날짜로, "+" 또는 부호가 없으면 이후 날짜로 이동하며 탐색한다.
* 예를 들어 두 숫자가 0,-5 라면, "오늘부터 시작해서 최근 5 개의 내용이 있는 날짜 페이지를 include" 하게 된다.
